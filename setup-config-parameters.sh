#!/bin/bash
# Script to set up config-specific parameters in AWS Systems Manager Parameter Store
# These parameters will be used by the config module
...
# Base path for config parameters
PARAM_PATH="/auxdromos/sit/config"
...
echo "Setting up config module parameters in AWS Systems Manager Parameter Store"
...
create_parameter "EXTERNAL_PORT" "8888" "String" "$OVERWRITE"
create_parameter "INTERNAL_PORT" "8888" "String" "$OVERWRITE"
create_parameter "SPRING_CLOUD_CONFIG_SERVER_GIT_URI" "https://gitlab.com/auxdromos/auxdromos-configuration.git" "String" "$OVERWRITE"
create_parameter "SPRING_CLOUD_CONFIG_SERVER_GIT_USERNAME" "oauth2" "String" "$OVERWRITE"
create_parameter "SPRING_CLOUD_CONFIG_SERVER_GIT_DEFAULT_LABEL" "master" "String" "$OVERWRITE"
create_parameter "SPRING_CLOUD_CONFIG_SERVER_GIT_SEARCH_PATHS" "configs/{application},configs/{application}/{profile}" "String" "$OVERWRITE"

# Create SecureString parameters
create_parameter "SPRING_CLOUD_CONFIG_SERVER_GIT_PASSWORD" "glpat-icte6rcutahRVZ_72Xv5" "SecureString" "$OVERWRITE"

echo "----------------------------------------------------"
echo "Config module parameter setup complete."
